...
$sql = "SELECT u.*, p.*, 
        (SELECT COUNT(*) FROM favorites WHERE talent_id = u.id) as favorite_count
        FROM users u 
...
// Get statistics
$sql = "SELECT views, downloads, favorites FROM statistics WHERE talent_id = ?";
if ($stmt = mysqli_prepare($conn, $sql)) {
    mysqli_stmt_bind_param($stmt, "i", $user_id);
    mysqli_stmt_execute($stmt);
    $stats_result = mysqli_stmt_get_result($stmt);
    $stats = mysqli_fetch_assoc($stats_result) ?: ['views' => 0, 'downloads' => 0, 'favorites' => 0];

    // Update view count
    $sql = "INSERT INTO statistics (talent_id, views) VALUES (?, 1) 
    ...
    if (!empty($comment)) {
        ...
        if ($stmt = mysqli_prepare($conn, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                // TODO: 等待notifications表创建后再启用
                /*
                // 发送通知给被评论用户
                $notification_sql = "INSERT INTO notifications (user_id, type, reference_id, message) 
                                   VALUES (?, 'comment', ?, ?)";
                if ($notif_stmt = mysqli_prepare($conn, $notification_sql)) {
                    $message = "Someone commented on your profile";
                    mysqli_stmt_bind_param($notif_stmt, "iis", $user_id, $user_id, $message);
                                ...
                                <?php while ($row = mysqli_fetch_assoc($resources)): ?>
                                    <div class="resource-card">
                                        ...
                                        <div class="resource-info">
                                            ...
                                            <a href="download-resource.php?id=<?php echo $row['id']; ?>"
